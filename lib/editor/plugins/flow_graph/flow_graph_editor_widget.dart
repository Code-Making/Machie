...
class FlowGraphEditorWidgetState
    extends EditorWidgetState<FlowGraphEditorWidget> {
  ...
  void _updateAssetDependencies() {
    ...
    final flowSettings =
        settings.pluginSettings[FlowGraphSettings] as FlowGraphSettings?;
    final schemaPath = flowSettings?.schemaPath ?? '';

    final newQueries = <AssetQuery>{};
    if (schemaPath.isNotEmpty) {
      newQueries.add(
        AssetQuery(path: schemaPath, mode: AssetPathMode.projectRelative),
      );
    }
        ...
        if (_isPaletteVisible)
          Positioned(
            ...
            child: NodePalette(
              ...
              onNodeSelected: (type) {
                final center =
                    _notifier.graph.viewportPosition * -1 +
