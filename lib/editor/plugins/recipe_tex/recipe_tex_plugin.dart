...
class RecipeTexPlugin extends EditorPlugin {
    ...
    BaseCommand(
      ...
      canExecute: (ref) {
        ...
        final isDirty = ref.watch(
          tabMetadataProvider.select((m) => m[tabId]?.isDirty ?? false),
        );
        return isDirty;
      },
    ),
  ...
  static List<Ingredient> _extractListItems(String content) {
    return RegExp(
      r'\\item\s+(.*?)\s*$',
      multiLine: true,
    ).allMatches(content).map((m) => _parseIngredient(m.group(1)!)).toList();
  }
  ...
  static List<InstructionStep> _extractInstructionItems(String content) {
    return RegExp(
      r'\\instruction\{(.*)\}\s*$',
      multiLine: true,
    ).allMatches(content).map((m) => _parseInstruction(m.group(1)!)).toList();
  }
