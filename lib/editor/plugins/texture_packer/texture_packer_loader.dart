...
class TexturePackerAssetLoader
    implements IDependentAssetLoader<TexturePackerAssetData> {
  @override
      ...
      void collectPaths(SourceImageNode node) {
        if (node.type == SourceNodeType.image && node.content != null) {
          if (node.content!.path.isNotEmpty) {
            ...
            final resolvedPath = repo.resolveRelativePath(
              tpackerPath,
              node.content!.path,
            );
  ...
  Future<TexturePackerAssetData> load(
      ...
      if (relativePath.isNotEmpty) {
        try {
          ...
          final assetData = await ref.read(
            assetDataProvider(resolvedPath).future,
          );
          ...
          ref
              ...
              .handle(
                e,
                st,
                'TexturePackerLoader: Failed to load source image $relativePath',
              );
        }
      }
    }
    ...
    void collectSprites(PackerItemNode node) {
      if (node.type == PackerItemType.sprite) {
        ...
        if (def is SpriteDefinition) {
          ...
          if (sourceImage != null) {
            final sourceConfig = _findSourceConfig(
              project.sourceImagesRoot,
              def.sourceImageId,
            );
    ...
    for (final item in result.items) {
      ...
      final sourceConfig =
          _findSourceConfig(project.sourceImagesRoot, def.sourceImageId)!;
