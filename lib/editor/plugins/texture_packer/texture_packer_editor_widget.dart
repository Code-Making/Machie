...
class TexturePackerEditorWidget extends EditorWidget {
  ...
  @override
  TexturePackerEditorWidgetState createState() =>
      TexturePackerEditorWidgetState();
}

class TexturePackerEditorWidgetState
    extends EditorWidgetState<TexturePackerEditorWidget> {
  ...
  void onSlicingGestureStart(Offset localPosition, SlicingConfig slicing) {
    ...
    final positionInImage = MatrixUtils.transformPoint(
      invMatrix,
      localPosition,
    );
  ...
  Future<void> _createSingleSprite(GridRect rect, String activeImageId) async {
    final spriteName = await showTextInputDialog(
      context,
    ...
    if (spriteName != null && spriteName.trim().isNotEmpty) {
      ...
      _notifier.updateSpriteDefinition(
        newNode.id,
  ...
  Future<void> _handleMultiSelection(
    GridRect rect,
    ...
    for (int y = 0; y < rect.height; y++) {
      for (int x = 0; x < rect.width; x++) {
        ...
        definitions.add(
          SpriteDefinition(sourceImageId: activeImageId, gridRect: tileRect),
        );
    ...
    if (choice == optionBatch) {
      ...
      final settings = ref.read(
        effectiveSettingsProvider.select(
          (s) =>
              s.pluginSettings[TexturePackerSettings] as TexturePackerSettings?,
        ),
      );
  ...
  Future<void> _promptAndAddSourceImages() async {
    ...
    final result = await showDialog<TexturePackerImportResult>(
      context: context,
      builder:
          (_) => TexturePackerFilePickerDialog(projectRootUri: project.rootUri),
    );
    ...
    for (final file in result.files) {
      final imageProjectRelativePath = repo.fileHandler.getPathForDisplay(
        file.uri,
        relativeTo: project.rootUri,
      );

      // Calculate path relative to the .tpacker file location
      final pathRelativeToTpacker = p
          .relative(imageProjectRelativePath, from: tpackerDirectory)
          .replaceAll(r'\', '/');
      ...
      if (result.asSprites) {
        try {
          ...
          final assetData = await ref.read(
            assetDataProvider(imageProjectRelativePath).future,
          );
          ...
          child: Card(
            child: Padding(
              ...
              child: CommandToolbar(
                position: TexturePackerPlugin.textureFloatingToolbar,
              ),
            ),
          ),
        ),
  ...
  Widget _buildMainContent() {
    ...
    return SlicingView(
      ...
      onGestureUpdate:
          (pos) => onSlicingGestureUpdate(pos, sourceConfig.slicing),
