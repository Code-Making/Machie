...
import '../../../../utils/texture_packer_algo.dart';
...
class PixiExportService {
  ...
  Future<void> export({
        ...
        if (def is SpriteDefinition) {
          ...
          if (sourceConfig != null) {
            ...
            if (image != null) {
              ...
              talker.warning(
                'Skipping sprite ${node.name}: Image not found for path "${sourceConfig.path}"',
              );
            }
          }
        }
    ...
    if (packerItems.isEmpty) {
      throw Exception(
        'No valid sprites found to export. Check source image paths.',
      );
    }
    ...
    for (final item in packedResult.items) {
      ...
      final nodeId =
          project.definitions.entries.firstWhere((e) => e.value == def).key;
    ...
    final atlasImage = await picture.toImage(
      packedResult.width.toInt(),
      packedResult.height.toInt(),
    );
    final pngBytes = await atlasImage.toByteData(
    ...
    final jsonOutput = {
      ...
      "meta": {
        ...
        "format": "RGBA8888",
        "size": {
          "w": packedResult.width.toInt(),
          "h": packedResult.height.toInt(),
        },
        "scale": "1",
      },
    };
