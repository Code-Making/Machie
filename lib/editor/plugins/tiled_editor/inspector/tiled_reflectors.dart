...
extension on Color {
  String toHex({String prefix = '#', bool includeAlpha = true}) {
    final r = r.toRadixString(16).padLeft(2, '0');
    final g = g.toRadixString(16).padLeft(2, '0');
    final b = b.toRadixString(16).padLeft(2, '0');
    if (includeAlpha) {
      final a = a.toRadixString(16).padLeft(2, '0');
      return '$prefix$a$r$g$b';
    }
    return '$prefix$r$g$b';
  }
...
class TiledReflector {
  ...
  static List<PropertyDescriptor> _getTiledObjectDescriptors(
    ...
    if (resolver != null) {
      ...
      final flowGraphParameters = resolver.getCachedFlowGraphParameters(
        flowGraphPath,
      );
      ...
      if (flowGraphParameters.isNotEmpty) {
        for (final param in flowGraphParameters) {
          ...
          void setter(dynamic v, PropertyType type) {
            final map = Map<String, Property<Object>>.from(
              obj.properties.byName,
            );
            Property<Object> newProp;
            switch (type) {
              case PropertyType.string:
                newProp = StringProperty(name: propName, value: v.toString());
                break;
              case PropertyType.int:
                newProp = IntProperty(name: propName, value: v as int);
                break;
              case PropertyType.float:
                newProp = FloatProperty(name: propName, value: v as double);
                break;
              case PropertyType.bool:
                newProp = BoolProperty(name: propName, value: v as bool);
                break;
              default:
                newProp = Property(name: propName, type: type, value: v);
                break;
            }
          ...
          switch (param.type) {
            ...
            case FlowPortType.number:
              descriptors.add(
                IntPropertyDescriptor(
                  ...
                  getter:
                      () =>
                          (obj.properties.getValue<num>(propName))?.toInt() ??
                          0,
            ...
            case FlowPortType.boolean:
              descriptors.add(
                BoolPropertyDescriptor(
                  ...
                  label: propLabel,
                  getter:
                      () => obj.properties.getValue<bool>(propName) ?? false,
            ...
            case FlowPortType.tiledObject:
              descriptors.add(
                TiledObjectReferencePropertyDescriptor(
                  name: propName,
                  label: propLabel,
                  map: map,
    ...
    if (obj.isPolygon) {
      descriptors.add(
        StringPropertyDescriptor(
          ...
          getter: () => obj.polygon.map((p) => '${p.x},${p.y}').join(' '),
          setter: (v) {},
          isReadOnly: true,
        ),
      );
    ...
    if (obj.isPolyline) {
      descriptors.add(
        StringPropertyDescriptor(
          ...
          getter: () => obj.polyline.map((p) => '${p.x},${p.y}').join(' '),
          setter: (v) {},
          isReadOnly: true,
        ),
      );
    ...
    if (obj.text != null) {
      descriptors.add(
        StringPropertyDescriptor(
          name: 'text_content',
          label: 'Text Content',
          getter: () => obj.text!.text,
          setter: (v) => obj.text!.text = v,
        ),
      );
    }
  ...
  static PropertyDescriptor _createMemberDescriptor(
    ...
    void setValue(dynamic val, PropertyType type) {
      ...
      switch (type) {
        ...
        case PropertyType.color:
          if (val is String) {
            newProp = ColorProperty(
              name: member.name,
              value: colorDataFromHex(val),
              hexValue: val,
            );
          } else {
            ...
            newProp = ColorProperty(
              name: member.name,
              value: colorData,
              hexValue: colorData.toHex(includeAlpha: true),
            );
          }
          break;
      }
    ...
    if (member.type == ClassMemberType.object) {
      ...
      final mapPropertyName = member.name
          .replaceFirst('Spawn', 'Map')
          .replaceFirst('Target', 'Map');
    ...
    if ((member.name == 'initialAnim' || member.name == 'initialFrame') &&
        resolver != null) {
      return DynamicEnumPropertyDescriptor(
        ...
        setter: (v) {
          talker?.info(
            "[Reflector] Setting ${member.name} to '$v' for Object ${obj.id}",
          );
        ...
        fetchOptions: () {
          ...
          if (atlasProp is! StringProperty) {
            talker?.debug(
              "[Reflector] 'atlas' property is missing or not a string.",
            );
            return [];
          }
  ...
  List<PropertyDescriptor> getDescriptors() {
    return [
      EnumPropertyDescriptor<RenderOrder>(
        ...
        label: 'Render Order',
        getter: () => renderOrder,
        setter: (v) => renderOrder = v,
        allValues: RenderOrder.values,
      ),
      IntPropertyDescriptor(
        name: 'width',
        label: 'Width (tiles)',
  ...
  List<PropertyDescriptor> getDescriptors() {
    final common = [
      ...
      DoublePropertyDescriptor(
        ...
        label: 'Opacity',
        getter: () => opacity,
        setter: (v) => opacity = v.clamp(0.0, 1.0),
      ),
      BoolPropertyDescriptor(
        name: 'visible',
        label: 'Visible',
      ...
      ColorPropertyDescriptor(
        ...
        label: 'Tint Color',
        getter: () => tintColorHex,
        setter: (v) => tintColorHex = v,
      ),
      DoublePropertyDescriptor(
        name: 'parallaxX',
        label: 'Parallax X',
        getter: () => parallaxX,
        setter: (v) => parallaxX = v,
      ),
      DoublePropertyDescriptor(
        name: 'parallaxY',
        label: 'Parallax Y',
        getter: () => parallaxY,
        setter: (v) => parallaxY = v,
      ),
    ];
    ...
    if (layer is ObjectGroup) {
      ...
      common.addAll([
        BoolPropertyDescriptor(
          name: 'repeatX',
          label: 'Repeat X',
          getter: () => layer.repeatX,
          setter: (v) => layer.repeatX = v,
        ),
        BoolPropertyDescriptor(
          name: 'repeatY',
          label: 'Repeat Y',
          getter: () => layer.repeatY,
          setter: (v) => layer.repeatY = v,
        ),
...
extension TilesetReflector on Tileset {
  List<PropertyDescriptor> getDescriptors() {
    return [
      ...
      EnumPropertyDescriptor<ObjectAlignment>(
        name: 'objectAlignment',
        label: 'Object Alignment',
        getter: () => objectAlignment,
        setter: (v) => objectAlignment = v,
        allValues: ObjectAlignment.values,
      ),
      ObjectPropertyDescriptor(
        name: 'image',
        label: 'Image',
    ...
    if (isPolygon) {
      descriptors.add(
        StringPropertyDescriptor(
          name: 'polygon',
          label: 'Polygon Points',
          getter: () => polygon.map((p) => '${p.x},${p.y}').join(' '),
          setter: (v) {},
          isReadOnly: true,
        ),
      );
    ...
    if (isPolyline) {
      descriptors.add(
        StringPropertyDescriptor(
          name: 'polyline',
          label: 'Polyline Points',
          getter: () => polyline.map((p) => '${p.x},${p.y}').join(' '),
          setter: (v) {},
          isReadOnly: true,
        ),
      );
    ...
    if (text != null) {
      ...
      descriptors.addAll([
        ...
        StringPropertyDescriptor(
          name: 'fontfamily',
          label: 'Font Family',
          getter: () => txt.fontFamily,
          setter: (v) => txt.fontFamily = v,
        ),
        IntPropertyDescriptor(
          name: 'pixelsize',
          label: 'Pixel Size',
          getter: () => txt.pixelSize,
          setter: (v) => txt.pixelSize = v,
        ),
        ColorPropertyDescriptor(
          name: 'color',
          label: 'Text Color',
          getter: () => txt.color,
          setter: (v) => txt.color = v,
        ),
        BoolPropertyDescriptor(
          name: 'wrap',
          label: 'Word Wrap',
        ...
        BoolPropertyDescriptor(
          ...
          label: 'Italic',
          getter: () => txt.italic,
          setter: (v) => txt.italic = v,
        ),
        BoolPropertyDescriptor(
          name: 'underline',
          label: 'Underline',
          getter: () => txt.underline,
          setter: (v) => txt.underline = v,
        ),
        BoolPropertyDescriptor(
          name: 'strikeout',
          label: 'Strikeout',
          getter: () => txt.strikeout,
          setter: (v) => txt.strikeout = v,
        ),
        BoolPropertyDescriptor(
          name: 'kerning',
          label: 'Kerning',
          getter: () => txt.kerning,
          setter: (v) => txt.kerning = v,
        ),
        EnumPropertyDescriptor<HAlign>(
          name: 'halign',
          label: 'Horizontal Align',
          getter: () => txt.hAlign,
          setter: (v) => txt.hAlign = v,
          allValues: HAlign.values,
        ),
        EnumPropertyDescriptor<VAlign>(
          name: 'valign',
          label: 'Vertical Align',
          getter: () => txt.vAlign,
          setter: (v) => txt.vAlign = v,
          allValues: VAlign.values,
        ),
      ]);
    }
