...
class _GlitchTabState {
  ...
  ui.Image? strokeSample; // For repeater brush
...
class GlitchEditorPlugin implements EditorPlugin {
  ...
  Widget buildToolbar(WidgetRef ref) {
    ...
    final isToolbarVisible = ref.watch(isToolbarVisibleProvider);
    return isToolbarVisible
        ? GlitchToolbar(plugin: this)
        : const BottomToolbar();
  }
  ...
  void applyGlitchStroke({
    ...
    // Push the pre-stroke image to the undo stack before replacing it
    final appNotifier = ref.read(appNotifierProvider.notifier);
    appNotifier.state.value?.currentProject?.session.currentTab?.plugin.undoStack.add(baseImage);
  ...
  List<Command> getCommands() => [
    BaseCommand(id: 'save', /* ... */),
    BaseCommand(id: 'save_as', /* ... */),
    BaseCommand(id: 'reset', /* ... */),
    ...
    BaseCommand(id: 'toggle_brush_settings', label: 'Brush Settings', icon: const Icon(Icons.brush), defaultPosition: CommandPosition.pluginToolbar, sourcePlugin: runtimeType.toString(),
      execute: (ref) async {
        ref.read(isToolbarVisibleProvider.notifier).update((state) => !state);
      },
    ),
  ];
