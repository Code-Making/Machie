...
class RecipeTexPlugin implements EditorPlugin {
  ...
  Widget buildEditor(EditorTab tab, WidgetRef ref) {
    return RecipeEditorForm(tab: tab as RecipeTexTab, plugin: this);
  }
  ...
  void _updateAndNotify(RecipeTexTab tab, WidgetRef ref) {
    final state = _tabStates[tab.file.uri];
    if (state == null) return;

    final isDirty = !const DeepCollectionEquality().equals(state.data, state.originalData);
    ref.read(tabStateProvider.notifier).state[tab.file.uri] = isDirty;
    
    // Trigger a rebuild by creating a new tab instance
    ref.read(appNotifierProvider.notifier).updateCurrentTab(tab.copyWith());
  }
  
  void updateDataForTab(RecipeTexTab tab, RecipeData Function(RecipeData) updater, WidgetRef ref) {
    final state = _tabStates[tab.file.uri];
    if (state == null) return;
    state.undoStack.add(state.data.copyWith());
    state.redoStack.clear();
    state.data = updater(state.data);
    _updateAndNotify(tab, ref);
  }

  void addIngredient(RecipeTexTab tab, WidgetRef ref) {
    updateDataForTab(tab, (d) => d.copyWith(ingredients: [...d.ingredients, Ingredient('', '', '')]), ref);
  }

  void removeIngredient(RecipeTexTab tab, int index, WidgetRef ref) {
    updateDataForTab(tab, (d) {
      final items = List.of(d.ingredients)..removeAt(index);
      return d.copyWith(ingredients: items);
    }, ref);
  }
  
  void reorderIngredient(RecipeTexTab tab, int oldIndex, int newIndex, WidgetRef ref) {
    updateDataForTab(tab, (d) {
  ...
  void addInstruction(RecipeTexTab tab, WidgetRef ref) {
    updateDataForTab(tab, (d) => d.copyWith(instructions: [...d.instructions, InstructionStep('', '')]), ref);
  }
  
  void removeInstruction(RecipeTexTab tab, int index, WidgetRef ref) {
    updateDataForTab(tab, (d) {
  ...
  void _undo(RecipeTexTab tab, WidgetRef ref) {
      final state = _tabStates[tab.file.uri];
      if (state == null || state.undoStack.isEmpty) return;
      state.redoStack.add(state.data);
      state.data = state.undoStack.removeLast();
      _updateAndNotify(tab, ref);
  }
  
  void _redo(RecipeTexTab tab, WidgetRef ref) {
      final state = _tabStates[tab.file.uri];
      if (state == null || state.redoStack.isEmpty) return;
      state.undoStack.add(state.data);
  ...
  List<Command> getCommands() => [
    BaseCommand(id: 'copy', label: 'Copy LaTeX', icon: const Icon(Icons.copy), defaultPosition: CommandPosition.pluginToolbar, sourcePlugin: runtimeType.toString(),
      execute: (ref) async {
        ...
        ref.read(rootScaffoldMessengerKeyProvider).currentState?.showSnackBar(const SnackBar(content: Text('Copied LaTeX to clipboard')));
      },
      canExecute: (ref) => ref.read(appNotifierProvider).value?.currentProject?.session.currentTab is RecipeTexTab,
    ),
    BaseCommand(id: 'save', label: 'Save Recipe', icon: const Icon(Icons.save), defaultPosition: CommandPosition.appBar, sourcePlugin: runtimeType.toString(),
      execute: (ref) async {
        ...
        _tabStates[tab.file.uri] = _RecipeTabState(data: state.data.copyWith(), originalData: state.data.copyWith(), undoStack: [], redoStack: []);
        ref.read(rootScaffoldMessengerKeyProvider).currentState?.showSnackBar(const SnackBar(content: Text('Recipe saved.')));
      },
    ...
    BaseCommand(id: 'undo', label: 'Undo', icon: const Icon(Icons.undo), defaultPosition: CommandPosition.pluginToolbar, sourcePlugin: runtimeType.toString(),
      execute: (ref) async {
        final tab = ref.read(appNotifierProvider).value?.currentProject?.session.currentTab as RecipeTexTab?;
        if (tab != null) _undo(tab, ref);
      },
    ...
    BaseCommand(id: 'redo', label: 'Redo', icon: const Icon(Icons.redo), defaultPosition: CommandPosition.pluginToolbar, sourcePlugin: runtimeType.toString(),
      execute: (ref) async {
        final tab = ref.read(appNotifierProvider).value?.currentProject?.session.currentTab as RecipeTexTab?;
        if (tab != null) _redo(tab, ref);
      },
