...
import '../plugins/recipe_tex/recipe_tex_plugin.dart';
...
// Global keys for showing dialogs/snackbars from the notifier
...
class AppNotifier extends AsyncNotifier<AppState> {
  ...
  Future<void> openProjectFromFolder({
    ...
    await _updateState((s) async {
      ...
      return s.copyWith(
        ...
        knownProjects: result.isNew ? [...s.knownProjects, result.metadata] : s.knownProjects,
      );
    });
    await saveAppState();
  }
  ...
  Future<OpenFileResult> openFile(DocumentFile file, {EditorPlugin? explicitPlugin}) async {
    ...
    // The `ref` passed to `project.openFile` is the AsyncNotifier's ref, which is correct.
    await _updateState((s) async {
    ...
    // After the project state is updated with the new tab, initialize its dirty state.
  ...
  // MODIFIED: Corrected the undefined variable error.
  void closeTab(int index) {
    final project = state.value?.currentProject;
    if (project == null) return;

    // Get the tab *before* updating the state.
    ...
    // Now use the captured tab object to remove its state.
